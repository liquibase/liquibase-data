...
class Migrate(
    ...
    fun migrate(container: String, name: String) {
        ...
        ui?.sendMessage("Creating repository $name")
        ...
        for (path in volumes.keys()) {
            ...
            ui?.sendMessage("Creating docker volume $volumeName with path $path")
            ...
            if (localSrc.isNotEmpty()) {
                ui?.sendMessage("Copying data to $volumeName")
        ...
        ui?.sendMessage("$container migrated to controlled environment $name")
    }
}
