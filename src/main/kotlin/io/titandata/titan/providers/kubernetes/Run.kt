...
class Run(
    ...
    fun run(
        ...
        ui?.sendMessage("Creating repository $repoName")
        ...
        try {
            for ((index, path) in volumes.keys().withIndex()) {
                ...
                ui?.sendMessage("Creating titan volume $volumeName with path $path")
            ...
            ui?.sendMessage("Waiting for volumes to be ready")
            var ready = false
            while (!ready) {
                ready = true
        ...
        ui?.sendMessage("Creating $repoName deployment")
        ...
        if (!disablePortMapping) {
            ui?.sendMessage("Forwarding local ports")
