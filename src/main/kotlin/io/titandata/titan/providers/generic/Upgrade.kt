...
class Upgrade(
    ...
    private fun downloadToTemp(binaryPath: String) {
        when {
            ...
            SystemUtils.IS_OS_WINDOWS -> {
                ui?.sendMessage("Not Yet Implemented")
            }
            else -> throw Exception("Operating system is unsupported.")
        }
    }
    ...
    fun upgrade(force: Boolean, version: String, finalize: Boolean, path: String?) {
        ...
        if (finalize) {
            ui?.sendMessage("Upgrading infrastructure")
            upgradeInfrastructure()

            ui?.sendMessage("Replacing Version")
        ...
        ui?.sendMessage("Downloading Latest Version")
        downloadToTemp(latestPath)

        ui?.sendMessage("Extracting Archive")
        extractTempToNew()

        ui?.sendMessage("Coping current bin to OLD")
