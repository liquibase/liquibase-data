...
class Docker(private val executor: CommandExecutor, val identity: String = "docker", val port: Int = 5001) {
    ...
    fun localcp(source: String, target: String): String {
        return executor.exec(listOf("docker", "cp", "-a", "$source/.", "titan-$identity-server:$target"))
    }

    fun cpOut(source: String, target: String): String {
        return executor.exec(listOf("docker", "cp", "-a", "titan-$identity-server:$source/.", target))
    }
